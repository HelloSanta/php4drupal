...
if [ ! -z "$SSH_KEY" ]; then
  ...
  echo "$SSH_KEY" > /myuser/.ssh/authorized_keys
fi

## Add id_ed25519 if exists.
if [ ! -z "$SSH_ID" ]; then
  mkdir -p /myuser/.ssh/
  echo "$SSH_ID" > /myuser/.ssh/id_ed25519
  sed -i 's/\\n/\
/g' /myuser/.ssh/id_ed25519 /myuser/.ssh/id_ed25519
  sed -i "s/\"//g" /myuser/.ssh/id_ed25519

  chmod 400 /myuser/.ssh/id_ed25519
fi
