...
RUN apt-get update && apt-get install -y openssh-server nano supervisor git && php -r "readfile('https://s3.amazonaws.com/files.drush.org/drush.phar');" > drush \
...
RUN apt-get install -y rsync mysql-client wkhtmltopdf
